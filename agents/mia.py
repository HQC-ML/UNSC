...
class MIA(object):
    def __init__(
        self,
        retain_loader_train, retain_loader_test, forget_loader, test_loader, model, device, num_classes=10, unlearn_class=3
    ):
        """
        ...
        shadow_train_performance = self.collect_performance(retain_loader_train, model, device)
        shadow_test_performance = self.collect_performance(test_loader, model, device)
        target_train_performance = self.collect_performance(retain_loader_test, model, device)
        target_test_performance = self.collect_performance(forget_loader, model, device)
    ...
    def _mem_inf_thre(self, v_name, s_tr_values, s_te_values, t_tr_values, t_te_values):
        ...
        t_tr_acc = t_tr_mem / (len(self.t_tr_labels) + 1e-5)
        t_te_acc = t_te_non_mem / (len(self.t_te_labels) + 1e-5)
        mem_inf_acc = 0.5 * (t_tr_acc + t_te_acc)
        print(
            "For membership inference attack via {n}, the attack acc is {acc1:.3f}, with train acc {acc2:.3f} and test acc {acc3:.3f}".format(
                n=v_name, acc1=mem_inf_acc, acc2=t_tr_acc, acc3=t_te_acc
    ...
    def mia(self,): 
        return self._mem_inf_benchmarks()
